...
def main():
    ...
    user_input = input("Enter the molecular formula of the sample:")
    sample_mass = float(input("Enter the mass in grams of the sample:"))

    symbol_quantity_list = parse_formula(user_input, elements)
    ...
    while continue_input.lower() != "no":
        continue_input = input("Make another calculation?(yes/no) ")
...
def compute_molar_mass(symbol_quantity_list, periodic_table):
    ...
    for _ in range(len(symbol_quantity_list)):
        symbol_key, quantity = symbol_quantity_list[_]
        quantity = int(quantity)
        periodic_data = periodic_table[symbol_key]

        #symbol_name = periodic_data[0]
        atomic_mass = float(periodic_data[1])
